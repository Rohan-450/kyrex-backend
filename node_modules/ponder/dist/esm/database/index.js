...
import { and, eq, getTableColumns, getTableName, is, lte, sql, } from "drizzle-orm";
...
`));
        ...
        async revert({ checkpoint, tx }) {
            await this.wrap({ method: "revert", includeTraceLogs: true }, () => Promise.all(tables.map(async (table) => {
                const primaryKeyColumns = getPrimaryKeyColumns(table);
                // @ts-ignore
                const { rows } = await tx.execute(sql.raw(`DELETE FROM "${namespace}"."${getTableName(getReorgTable(table))}" WHERE checkpoint > '${checkpoint}' RETURNING *`));
                ...
                for (const log of reversed) {
                    if (log.operation === 0) {
                        // create
                        await tx.delete(table).where(and(...primaryKeyColumns.map(({ js, sql }) => 
                        // @ts-ignore
                        eq(table[js], log[sql]))));
                    }
                        ...
                        await tx
                            .update(table)
                            .set(log)
                            .where(and(...primaryKeyColumns.map(({ js, sql }) => 
                        // @ts-ignore
                        eq(table[js], log[sql]))));
                    }
                    else {
