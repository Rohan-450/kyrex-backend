...
export const getCacheKey = (table, key) => {
...
export const createIndexingCache = ({ common, database, schemaBuild: { schema }, checkpoint, }) => {
    ...
    return {
        ...
        get({ table, key, db }) {
            ...
            const bufferEntry = updateBuffer.get(table).get(getCacheKey(table, key)) ??
                insertBuffer.get(table).get(getCacheKey(table, key));
            if (bufferEntry) {
                return structuredClone(bufferEntry.row);
            }
            const entry = spillover.get(table).get(getCacheKey(table, key)) ??
        ...
        async flush({ db }) {
            for (const table of cache.keys()) {
                const batchSize = Math.round(common.options.databaseMaxQueryParameters /
                ...
                if (insertValues.length > 0) {
                    for (const insertChunk of chunk(insertValues, batchSize)) {
                        await database.record({ method: `${getTableName(table)}.flush()` }, async () => {
                            await db
                                .insert(table)
                ...
                if (updateValues.length > 0) {
                    ...
                    const set = {};
                    for (const [columnName, column] of Object.entries(getTableColumns(table))) {
                        set[columnName] = sql.raw(`excluded."${getColumnCasing(column, "snake_case")}"`);
                    }
