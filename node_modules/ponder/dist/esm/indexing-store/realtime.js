...
export const createRealtimeIndexingStore = ({ schemaBuild: { schema }, database, }) => {
    ...
    return {
        // @ts-ignore
        find: (table, key) => queue.add(() => database.wrap({ method: `${getTableName(table) ?? "unknown"}.find()` }, async () => {
        ...
        insert(table) {
            return {
                values: (values) => {
                    // @ts-ignore
                    const inner = {
                        onConflictDoNothing: () => queue.add(() => database.wrap({
                        ...
                        onConflictDoUpdate: (valuesU) => queue.add(() => database.wrap({
                            method: `${getTableName(table) ?? "unknown"}.insert()`,
                        }, async () => {
                            checkOnchainTable(table, "insert");
        ...
        update(table, key) {
            return {
                set: (values) => queue.add(() => database.wrap({ method: `${getTableName(table) ?? "unknown"}.update()` }, async () => {
        ...
        // @ts-ignore
        delete: (table, key) => queue.add(() => database.wrap({ method: `${getTableName(table) ?? "unknown"}.delete()` }, async () => {
        ...
        // @ts-ignore
        sql: drizzle((_sql, params, method, typings) => 
        // @ts-ignore
        queue.add(async () => {
