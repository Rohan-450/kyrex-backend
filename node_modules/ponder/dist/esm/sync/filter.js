...
const isValueMatched = (filterValue, eventValue) => {
    ...
    if (filterValue instanceof Set &&
        filterValue.has(toLowerCase(eventValue))) {
        return true;
    }
    // single
 ...
 */
export const isLogFactoryMatched = ({ filter, log, }) => {
 ...
 * Returns `true` if `log` matches `filter`
 */
export const isLogFilterMatched = ({ filter, block, log, childAddresses, }) => {
    // Return `false` for out of range blocks
    if (hexToNumber(block.number) < (filter.fromBlock ?? 0) ||
    ...
    if (isAddressFactory(filter.address)) {
        if (Array.isArray(childAddresses)) {
            if (childAddresses.every((address) => isValueMatched(address, log.address) === false)) {
                return false;
            }
        }
        else {
 ...
 */
export const isTransactionFilterMatched = ({ filter, block, transaction, fromChildAddresses, toChildAddresses, }) => {
    // Return `false` for out of range blocks
    if (hexToNumber(block.number) < (filter.fromBlock ?? 0) ||
        hexToNumber(block.number) > (filter.toBlock ?? Number.POSITIVE_INFINITY)) {
        return false;
    ...
    if (isAddressFactory(filter.fromAddress)) {
        if (Array.isArray(fromChildAddresses)) {
            if (fromChildAddresses.every((address) => isValueMatched(address, transaction.from) === false)) {
                return false;
            }
        }
        else {
        ...
        if (isValueMatched(filter.fromAddress, transaction.from) === false) {
            return false;
        }
    }
    if (isAddressFactory(filter.toAddress)) {
        if (Array.isArray(toChildAddresses)) {
            if (transaction.to !== null &&
                toChildAddresses.every((address) => isValueMatched(address, transaction.to) === false)) {
                return false;
            }
        }
        else {
 ...
 */
export const isTraceFilterMatched = ({ filter, block, trace, fromChildAddresses, toChildAddresses, }) => {
    // Return `false` for out of range blocks
    if (hexToNumber(block.number) < (filter.fromBlock ?? 0) ||
        hexToNumber(block.number) > (filter.toBlock ?? Number.POSITIVE_INFINITY)) {
        return false;
    ...
    if (isAddressFactory(filter.fromAddress)) {
        if (Array.isArray(fromChildAddresses)) {
            if (fromChildAddresses.every((address) => isValueMatched(address, trace.from) === false)) {
                return false;
            }
        }
        else {
        ...
        if (isValueMatched(filter.fromAddress, trace.from) === false) {
            return false;
        }
    }
    if (isAddressFactory(filter.toAddress)) {
        if (Array.isArray(toChildAddresses)) {
            if (toChildAddresses.every((address) => isValueMatched(address, trace.to) === false)) {
                return false;
            }
        }
        else {
        ...
        if (isValueMatched(filter.toAddress, trace.to) === false) {
            return false;
        }
    }
 ...
 * Returns `true` if `trace` matches `filter`
 */
export const isTransferFilterMatched = ({ filter, block, trace, fromChildAddresses, toChildAddresses, }) => {
    // Return `false` for out of range blocks
    if (hexToNumber(block.number) < (filter.fromBlock ?? 0) ||
        hexToNumber(block.number) > (filter.toBlock ?? Number.POSITIVE_INFINITY)) {
        return false;
