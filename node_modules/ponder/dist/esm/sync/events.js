...
import { isBlockFilterMatched, isLogFilterMatched, isTraceFilterMatched, isTransactionFilterMatched, isTransferFilterMatched, } from "./filter.js";
import { isAddressFactory, shouldGetTransactionReceipt } from "./filter.js";
/**
 * Create `RawEvent`s from raw data types
 */
export const buildEvents = ({ sources, blockWithEventData: { block, logs, transactions, transactionReceipts, traces, }, finalizedChildAddresses, unfinalizedChildAddresses, chainId, }) => {
    const events = [];
    ...
    for (const transactionReceipt of transactionReceipts) {
        transactionReceiptCache.set(transactionReceipt.transactionHash, transactionReceipt);
    }
                            ...
                            if (isLogFilterMatched({ filter, block, log }) &&
                                ...
                                events.push({
                                    ...
                                    transaction: transactionCache.has(log.transactionHash)
                                        ? convertTransaction(transactionCache.get(log.transactionHash))
                                        : undefined,
                                    transactionReceipt: transactionReceiptCache.has(log.transactionHash) &&
                                        shouldGetTransactionReceipt(filter)
                                        ? convertTransactionReceipt(transactionReceiptCache.get(log.transactionHash))
                                        : undefined,
                                    trace: undefined,
                                });
                            }
                    ...
                    case "trace": {
                        for (const trace of traces) {
                            ...
                            if (isTraceFilterMatched({
                                ...
                                const transactionReceipt = transactionReceiptCache.get(trace.transactionHash);
                                events.push({
                                    chainId: filter.chainId,
                                    sourceIndex: i,
                                    checkpoint: encodeCheckpoint({
                        ...
                        for (const transaction of transactions) {
                            ...
                            if (isTransactionFilterMatched({
                                ...
                                events.push({
                                    ...
                                    block: convertBlock(block),
                                    transaction: convertTransaction(transaction),
                                    transactionReceipt: convertTransactionReceipt(transactionReceiptCache.get(transaction.hash)),
                                });
                            }
                        }
                        break;
...
export function removeNullCharacters(obj) {
...
const convertTransactionReceipt = (transactionReceipt) => ({
