...
export const createSync = async (params) => {
    ...
    async function* getEvents() {
        ...
        const eventGenerators = Array.from(perNetworkSync.entries()).map(([network, { syncProgress, historicalSync }]) => {
            ...
            return bufferAsyncGenerator(decodeEventGenerator(localEventGenerator), 1);
        });
    ...
    if (params.ordering === "multichain") {
        for (const network of params.indexingBuild.networks) {
            seconds[network.name] = {
                ...
                cached: decodeCheckpoint(min(getOmnichainCheckpoint({ tag: "end" }), getOmnichainCheckpoint({ tag: "finalized" }), params.initialCheckpoint)).blockTimestamp,
            };
        }
    }
    else {
        ...
        async startRealtime() {
            for (const network of params.indexingBuild.networks) {
                ...
                if (to > from) {
                    ...
                    const decodedEvents = decodeEvents(params.common, sources, events.events);
                    params.common.logger.debug({
                        service: "sync",
